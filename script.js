...
// 하이라이트 함수: 선택된 과목(selectedSubject)에만 'selected-math' 클래스 부여
function highlightMathSubjects(text, colorClass, selectedSubject = null) {
    ...
    sortedPatterns.forEach(item => {
        highlighted = highlighted.replace(item.regex, (match) => {
            // 요약표에서 클릭한 이름과 현재 패턴 이름이 정확히 일치할 때만 selected-math 추가
...
function searchMajor(selectedSubject = null) {
    ...
    const aliasEntry = aliasData.find(row => {
        ...
        return aliases.includes(query) || (row["대표전공"] === query);
    });
    ...
    // 필터링: 국어교육 검색 시 중국어교육 등 노이즈 차단
    const results = majorData.filter(row => {
        const dept1 = (row["모집단위1"] || "").trim();
        const dept2 = (row["모집단위2"] || "").trim();
        const fullDept = dept1 + " " + dept2;
        
        // 검색어가 학과명에 직접 포함되어 있는가? (예: '국어교육')
        const containsDirect = fullDept.includes(query);
        // 별칭 키워드가 포함되어 있는가?
        const containsKeyword = searchKeywords.some(k => fullDept.includes(k));
        
        // 검색어 자체가 학과명에 있다면 그 결과만 우선적으로 보여줌 (노이즈 방지)
        if (query.length >= 2 && containsDirect) return true;
                ...
                <div class="summary-tags">
                    ${sortedMath.map(([name, count]) => {
                        ...
                        return `<span onclick="filterBySubject('${name}')" 
                                      style="cursor:pointer; ${isThisSelected ? 'background:#2563eb; color:white; font-weight:bold; border:2px solid #1e40af;' : 'background:white;'}">
...
async function init() {
    try {
        ...
        document.getElementById("subjectInput").onkeydown = (